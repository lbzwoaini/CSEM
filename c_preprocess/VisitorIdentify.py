    ...
    def dealExpression(self, expression):
        ...
        if isinstance(expression, c_ast.ID):
            ...
            if self.idType == "funcName":
                ...
                result = ["identify"]
            if metaMap.get(result[0]) is None:
                metaMap[result[0]] = 0
            else:
                metaMap[result[0]] += 1
            return result
        ...
        if isinstance(expression, c_ast.Decl):
            ...
            nameList = self.dealExpression(name)
            result = []
...
tempOutDir = "../temp_out/"
if __name__ == "__main__":
    inputFilePath = "../OJ_CLONE_217_FILTERED/"
    outFilePath = "../OJ_CLONE_217_IDEN_FILTERED/"
    for questionName in os.listdir(inputFilePath):
        ccNum = 0
        questionDir = inputFilePath + questionName + "/"
        for codeFileName in os.listdir(questionDir):
...
#    inputFilePath = "../题目分组/"
#    outFilePath = "../1121/"
#    dealFile(generateTempFile(r"../题目分组/972297349079040000_查找整数/16-计算机-胡跃澍_972453970576543744_972297349079040000.c", tempOutDir), "../test_out_dir/")